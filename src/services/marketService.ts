...
import { getCachedMerchants, loadMerchants } from './contentLoader';
...
/** 获取当前商家数据源（优先 CMS JSON，降级硬编码） */
function getMerchantDataSource(): MarketMerchant[] {
  return getCachedMerchants();
}

// 导出硬编码降级数据
export { SAMPLE_MARKET_MERCHANTS } from './marketServiceTypes';
...
export async function fetchMarketMerchants(params: FetchMarketMerchantsParams): Promise<PaginatedMerchants> {
  ...
  let filtered = getMerchantDataSource().filter(m => m.market === params.market);
...
export async function fetchFactoryDetail(id: string): Promise<FactoryDetail> {
  ...
  const merchant = getMerchantDataSource().find(m => m.id === id);
